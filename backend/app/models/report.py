...
class Report(Base):
    ...
    status: Mapped[ReportStatus] = mapped_column(SAEnum(ReportStatus), default=ReportStatus.draft, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    photos: Mapped[List["Photo"]] = relationship(back_populates="report", cascade="all, delete-orphan")
    tasks: Mapped[List["Task"]] = relationship(back_populates="report", cascade="all, delete-orphan")
    signature: Mapped[Optional["Signature"]] = relationship(back_populates="report", uselist=False, cascade="all, delete-orphan")
...
class Photo(Base):
    ...
    priority: Mapped[PhotoPriority] = mapped_column(SAEnum(PhotoPriority), default=PhotoPriority.none)
...
class Task(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    report_id: Mapped[int] = mapped_column(ForeignKey("reports.id", ondelete="CASCADE"), index=True)
    photo_id: Mapped[Optional[int]] = mapped_column(ForeignKey("photos.id", ondelete="SET NULL"), index=True)
